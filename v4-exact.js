...
function init(){
 ...
 // Product-card variant carousel foundation. Future products can expose p.images=[...]; the UI already supports swipe/arrow switching.
 ...
 function enhanceCards(){ $$('.card',grid).forEach(card=>{if(card.dataset.v4)return;card.dataset.v4='1';const img=$('.photo img',card);if(!img)return;const id=(img.src.split('/').pop()||'').split('.')[0];const variants=window.BENNI_VARIANTS[id]||[img.getAttribute('src')];if(variants.length<2)return;let idx=0;const photo=$('.photo',card);const dots=document.createElement('div');dots.className='v4-color-dots';variants.forEach((src,i)=>{const b=document.createElement('button');b.setAttribute('aria-label','Colour '+(i+1));b.style.cssText='width:10px;height:10px;border-radius:50%;border:1px solid #aaa;background:#'+['111','7b1e2b','d8c3a0','eee'][i%4]+';padding:0;margin:0 3px';b.onclick=e=>{e.stopPropagation();idx=i;img.src=src};dots.appendChild(b)});photo.parentNode.insertBefore(dots,photo.nextSibling)});}
}
